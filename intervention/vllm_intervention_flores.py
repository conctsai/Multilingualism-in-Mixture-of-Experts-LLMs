...
from dataloader import load_flores_dataset
...
def infer(lang, model_path, seed):
    ...
    dataset = load_flores_dataset(language="eng_Latn")
    ...
    messages = [
        {
            "role": "user",
            "content": f"Translate the following English text into {FLORES_TO_LANG[lang]}.\n" + f"English: {data['text']}\n\n" + f"{FLORES_TO_LANG[lang]}:"
        } for data in dataset
    ]
...
def main():
    ...
    for lang in langs:
        ...
        json.dump(results, open(f"intervention/flores_output/flores_target_{args.target_lang}_lang_{lang}_seed{args.seed}_start{args.start_layer}_end{args.end_layer}_k{args.k}_thr{args.thr}_shared{args.shared}.json", "w"), indent=4, ensure_ascii=False)
        ...
        for res in results:
            actual_lang = detect_langs(res['prediction'])
            expected_lang = lang
            if expected_lang == "zh":
                langs_detected = set([res.lang.lower() for res in actual_lang])
                if langs_detected == {"zh-cn", "ko"}:
                    lang_consistency += 1
                    continue
            if len(actual_lang) == 2:
                lang_consistency += 0
                continue
            elif actual_lang[0].lang.lower() == expected_lang:
                lang_consistency += 1
            else:
                lang_consistency += 0
        ...
        out_path = "intervention/flores_result.jsonl"
        result = {
            "language": lang,
