...
from dataloader import load_xquad_dataset
from transformers import AutoTokenizer

direct_answer_dic = {
    "en": "Please output the answer directly.",
    "zh": "请直接输出答案。",
    "ar": "يُرجى إخراج الإجابة مباشرة.",
    "de": "Bitte geben Sie die Antwort direkt aus.",
    "es": "Por favor, proporcione la respuesta directamente.",
    "ru": "Пожалуйста, выведите ответ напрямую.",
    "th": "กรุณาแสดงคำตอบโดยตรง",
    "vi": "Vui lòng xuất câu trả lời trực tiếp.",
    "el": "Παρακαλώ δώστε την απάντηση απευθείας.",  # 希腊语
    "hi": "कृपया सीधे उत्तर दें।",                   # 印地语
    "ro": "Vă rugăm să răspundeți direct.",          # 罗马尼亚语
    "tr": "Lütfen cevabı doğrudan verin."            # 土耳其语
}
...
def infer(lang, model_path, seed):
    ...
    dataset = load_xquad_dataset(language=lang)

    messages = [
        {
            "role": "user",
            "content": data['context'] + '\n' +  data['question'] + '\n' + direct_answer_dic[lang]
        } for data in dataset
    ]
...
def main():
    ...
    print(f"Ablation data saved to intervention/intervention_data.json")
    ...
    for lang in langs:
        ...
        json.dump(results, open(f"intervention/xquad_output/xquad_target_{args.target_lang}_lang_{lang}_seed{args.seed}_start{args.start_layer}_end{args.end_layer}_k{args.k}_thr{args.thr}_shared{args.shared}.json", "w"), indent=4, ensure_ascii=False)
        ...
        out_path = "intervention/xquad_result.jsonl"
        result = {
            "language": lang,
